...
def compute_negative_laplacian(Adj):
    ...
    # Here is Akshat's awesome one liner:
...
def get_pi_frac(a,b):
    ...
    # Convert time to a fraction of a/b π
...
def create_wheel_graph(N):
    """
    Create a wheel graph with N+1 nodes: one central node connected to N peripheral nodes
    which are also connected in a cycle.
    """
    G = nx.Graph()
    center = 0  # Central node
    G.add_node(center)
    G.nodes[center]['generation'] = 0  # Mark the central node

    # Add peripheral nodes and connect them to the central node
    for i in range(1, N+1):
        G.add_node(i)
        G.nodes[i]['generation'] = 1  # Peripheral nodes
        G.add_edge(center, i)

    # Connect peripheral nodes in a cycle
    for i in range(1, N+1):
        G.add_edge(i, (i % N) + 1)
    return G

def plot_wheel_graph(G):
